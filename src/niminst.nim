...
type
  ...
  ConfigData = object of RootObj
    ...
    ccompiler, linker, nsisSetup: tuple[path, flags: string]
    innosetup: tuple[path, flags, installerName: string]
...
proc innoPathFlags(p: var CfgParser, k, v: string,
                   t: var tuple[path, flags, installerName: string]) =
  case normalize(k)
  of "path": t.path = v
  of "flags": t.flags = v
  of "installername": t.installerName = v
  else: quit(errorStr(p, "unknown variable: " & k))
  ...
  if input != nil:
    ...
    while true:
      ...
      of cfgKeyValuePair:
        ...
        of "innosetup": innoPathFlags(p, k.key, v, c.innosetup)
